...
* rects: `xmin_`, `xmax_`, `ymin_`, `ymax_`.
* ribbons: `x_`, `ymin_`, `ymax_`.
* arcs: `x_`, `y_`, `rmin_`, `rmax_`, `thetamin_`, `thetamax_`.
...
* All
    * `geometry_rotate()` (except ribbons).
    * `geometry_flip(axis = "x")`
  
  * Paths/polygons
    * `geometry_interpolate()`
    * `geometry_transform()` (would use [adaptive resampling](http://bl.ocks.org/mbostock/3795544))
    * `geometry_simplify()`
    * `geometry_jitter()`
