...
All gggeom manipulations should be able to proces s ~100,000 geometries in less than 0.1. More geometries than that is unlikely to produce a useful plot. If you do have very large datasets, they should be summarised (using e.g. the tools [ggstat](https://github.com/rstudio/ggstat)) before being visualised.
...
All geometries have a base graphics `plot()` method. These are useful for examples, explanation and debugging, but not serious data visualisation. `...` is passed on to the underlying base graphic method, so if you're familiar with the graphic **par**ameters, you can tweak the appearance. 
